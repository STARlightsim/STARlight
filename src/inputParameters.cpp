...
inputParameters::inputParameters()
	: _baseFileName("baseFileName", "slight"),
	  ...
	  _lambdaMass("lambdaMass", 1.115683, NOT_REQUIRED),
	  ...
	  _rho0PrimeBrFourProng("rho0PrimeBrFourProng", 1.0, NOT_REQUIRED),
	  _rho0PrimeCoupling("rho0PrimeCoupling", 8.4, NOT_REQUIRED),
	  ...
	  _SigmaNNInel("SigmaNNInel", -1, NOT_REQUIRED)
{
	...
	_ip.addParameter(_rho0PrimeBrFourProng);
	_ip.addParameter(_rho0PrimeCoupling);
...
bool inputParameters::init()
{
	...
	case 999: // pi+pi-pi+pi- phase space decay
		...
		{
			defaultMaxW = 10.;
		} // SRK May 29, 2019, to avoid an unduly high defaultMaxW
		_inputBranchingRatio = rho0PrimeBrFourProng();
		break;
	case 223: // omega(782)
		_particleType = OMEGA;
