	  ...
	  _sigmaNucleus(_bbs.beam2().A())
{
	...
	case FOURPRONG:
		_slopeParameter = 9.4;
		_vmPhotonCoupling = _ip->rho0PrimeCoupling();
		//_vmPhotonCoupling      = 2.02 * (_ip->rho0PrimeMass()/_ip->rho0Mass()) * (_ip->rho0PrimeMass()/_ip->rho0Mass());
		_ANORM = -2.75;
		_BNORM = 0;
		_defaultC = 1.0;
								   ...
								   const int beam)
{
	...
	Wgp = sqrt(2. * Egamma * (_protonEnergy + sqrt(_protonEnergy * _protonEnergy - _ip->protonMass() * _ip->protonMass())) + _ip->protonMass() * _ip->protonMass());
...
photonNucleusCrossSection::photonFlux(const double Egamma, const int beam)
{
	...
	if (Icheck > 1 && _bbs.beam1().A() == _bbs.beam2().A() && _bbs.beam1().Z() == _bbs.beam2().Z())
		goto L1000f;
...
photonNucleusCrossSection::sigmagp(const double Wgp)
{
	...
	case RHOZEUS:
		...
		break;
	case FOURPRONG:
		sigmagp_r = (1. / _ip->rho0PrimeBrFourProng()) * 1.E-4 * (0.26 * exp(0.28 * log(Wgp)) + 20.78 * exp(-1.21 * log(Wgp)));
		break;
	case OMEGA:
	case OMEGA_pipipi:
									   ...
									   const double C)
{
	...
	if (_particleType == JPSI_lambdalambdabar)
	{
		if (W < 2. * _ip->lambdaMass())
		{
			nrbw_r = 0.;
			return nrbw_r;
		}
		ppi = sqrt(((W / 2.) * (W / 2.)) - _ip->lambdaMass() * _ip->lambdaMass());
		ppi0 = sqrt(((_channelMass / 2.) * (_channelMass / 2.)) - _ip->lambdaMass() * _ip->lambdaMass());
	}
